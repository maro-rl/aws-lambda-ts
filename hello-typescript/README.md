...
](https://aws.amazon.com/es/blogs/compute/building-typescript-projects-with-aws-sam-cli/)
...
- [Create a Simple AWS Lambda Using TypeScript](https://betterprogramming.pub/create-simple-aws-lambda-using-typescript-732c8a4a7a09)
...
```bash
hello-typescript$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
hello-typescript$ sam local start-api
hello-typescript$ curl http://localhost:3000/
```
...
```bash
hello-typescript$ sam logs -n HelloWorldFunction --stack-name hello-typescript --tail
```
...
```bash
hello-typescript$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name hello-typescript
```
